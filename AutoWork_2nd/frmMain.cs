    ...
    public partial class frmMain : Form
    {
        ...
        public frmMain()
        {
            ...
            try
            {
                ...
                //sLuckNum = myConfig["LuckNum").Split(new char[] { '|', '@' }, StringSplitOptions.RemoveEmptyEntries);//获取幸运数字数组
                sLuckNum = myConfig["LuckNum"].ToString().Split('|');//格式  33333333@888@88888|3@1@88 
                gameNames = myConfig["gameName"].ToString().Split('|');
                rolltimes = Array.ConvertAll(myConfig["RollTimes"].ToString().Split(new char[] { '|', '@' }, StringSplitOptions.RemoveEmptyEntries), int.Parse);

                Act4Stander = Array.ConvertAll(myConfig["Act4Stander"].ToString().Split(new char[] { '|', '@' }, StringSplitOptions.RemoveEmptyEntries), int.Parse);

                Act4Set = Array.ConvertAll(myConfig["Act4Set"].ToString().Split(new char[] { '|', '@' }, StringSplitOptions.RemoveEmptyEntries), int.Parse);
                ...
                mailbody = myConfig["mailbody"].ToString().Split(new char[] { '|', '@' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public class MyJob2 : IJob
        {
            public void Execute(IJobExecutionContext context)
            {
                ...
                switch (myConfig["GetDataType"].ToString())
                {
                    case "1":
                        list = platACT.getActData(actInfo[0]);
                        break;
                    case "2":
                        list = ActFromDB.getActData_old(actInfo[0]);
                        break;
                    case "3":
                        list = ActFromDB.getActData(actInfo[0]);
                        break;
                }
                ...
                foreach (var item in list)
                {
                    ...
                    string sql = "select * from record where (betno='" + bb.betno + "' and pass=1 and aid=" + bb.aid + " ) or (pass=1  and aid=" +bb.aid + " and  username='" + bb.username + "' and gamename='" + bb.gamename + "'  and subtime > '" + DateTime.Now.AddHours(-12).ToString("yyyy-MM-dd") + " 00:00:01' and  subtime < '" + DateTime.Now.AddHours(-12).ToString("yyyy-MM-dd") + " 23:59:59') ";
            ...
            public void Execute(IJobExecutionContext context)
            {
                ...
                foreach (var item in list)
                {
                    ...
                    if (item.betno != "" && Regex.IsMatch(item.betno, @"^\d{12}$"))
                    {
                        ...
                        string sql = "select * from record where (betno='" + bb.betno + "' and pass=1 and aid =" + bb.aid + ") or ( pass=1 and aid =" + bb.aid + " and username='" + bb.username + "'   and subtime > '" + DateTime.Now.AddHours(-12).ToString("yyyy-MM-dd") + " 00:00:01' and  subtime < '" + DateTime.Now.AddHours(-12).ToString("yyyy-MM-dd") + " 23:59:59') ";
                        if (appSittingSet.recorderDbCheck(sql))
                        {
                            bb.passed = false;
                            bb.msg = "您好，同一游戏一天内只能申请一次，申请不通过！R";
        ...
        public class MyJob4 : IJob
        {
            ...
            {
                //获取注单号码  yh
                List<betData> list = platACT.getActData(actInfo[6]);
                if (list == null)
                {
                ...
                foreach (var item in list)
                {
                    ...
                    bb.aname= actInfo[7];
                    //判断是否提交过 同一用户 所有游戏 一天只能一次
                    string sql = "select * from record where pass=1 and aid =" +bb.aid + " and LOWER(username)='" + bb.username.ToLower() + "'   and subtime > '" + DateTime.Now.AddHours(-12).ToString("yyyy-MM-dd") + " 00:00:01' and  subtime < '" + DateTime.Now.AddHours(-12).ToString("yyyy-MM-dd") + " 23:59:59' ";
                    if (appSittingSet.recorderDbCheck(sql))
                    {
                        bb.passed = false;
        ...
        public class MyJob11 : IJob
        {
            public void Execute(IJobExecutionContext context)
            {
                ...
                //MySQLHelper.connectionString = myConfig["MySqlConnect").Split('|')[0];
                foreach (var item in list)
                {
                    //item.aid = actInfo[21];
                    item.aname = actInfo[22];
        ...
        public class MyJob12 : IJob
        {
            public void Execute(IJobExecutionContext context)
            {
                ...
                List<betData> list = ActFromDB.getActData_old(actInfo[18]);

                //if (ActFromDB.list == null)
                //{
                //    MyWrite(actInfo[19] + " 没有获取到新的注单，等待下次执行 ");
                //    return;
                //}
                ...
                foreach (var item in list)
                {
                    ...
                    item.aname = actInfo[19];
                    //判断是否已经处理过
                    betData t1 = appSittingSet.getOneFromLocDbByID(item.bbid);
                    if (t1!=null)
                    {
                        ActFromDB.confirmAct_old(t1);
                        continue;
                    }
                    ...
                    //bool fr = platGPK.MemberDepositSubmit(item);
                    bool fr = true;
                        MyWrite($"用户{item.username}处理，存入金额{item.betMoney} ,活动名称{item.aname} ");
                        appSittingSet.Log($"用户{item.username}处理，存入金额{item.betMoney} ,活动名称{item.aname} ");
                    if (fr)
                    {
                        string msg = $"用户{item.username}处理，存入金额{item.betMoney} ,活动名称{item.aname} ";
                        MyWrite(msg);
                        appSittingSet.Log(msg);
