    ...
    public static class platQPG
    {
        ...
        public static List<betData> getActData()
        {
            ...
            try
            {
                ...
                ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;
                string url = urlbase + $"v2/api/merchant/merchantcenter/oc/qryMerchantOrderByCondition?tradeId=&timeStart={DateTime.Now.Date.ToString("yyyy-MM-dd")}+00:00:00&timeEnd={DateTime.Now.Date.ToString("yyyy-MM-dd")}+23:59:59&payStatus=PAYED&synStatus=NOT_DEAL&fastSubName=&currentPage=1&pageSize=10";
                ...
                if (jo["message"].ToString() == "操作成功")
                {
                    ...
                    foreach (var item in ja)
                    {
                        ...
                        b.bbid = item["tradeNo"].ToString().Trim();
                        b.username = item["fastSubName"].ToString().Trim();
                        b.betMoney = decimal.Parse(item["orderAmountRmb"].ToString().Trim()) / 10000;

                        //if (item["originTradeUq"].ToString().Trim().Length>0)
                        //    b.Memo = "补单" + item["tradeNo"].ToString().Trim();
                        //else
                        //    b.Memo = item["tradeNo"].ToString().Trim();

                        if (item["synDesc"].ToString().Trim().Length == 0)
                            list.Add(b);
        ...
        public static bool confirmAct(betData b)
        {
            ...
            if (r)
            {
                ...
                string msg = $"订单号{b.bbid}用户{b.username}处理完毕，处理为 {(b.passed ? "通过" : "不通过")}，回复消息 {b.msg} {DateTime.Now.ToString()}";
        ...
        public static bool addRemark(betData b)
        {
            try
            {
                /*
                ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;
                string url = urlbase + $"v2/api/merchant/merchantcenter/oc/remarkMerchantOrder?tradeNo={b.bbid}&desc={HttpUtility.UrlEncode(b.msg,Encoding.UTF8).ToUpper()}";
                ...
                */
                ...
                string postdata = JsonConvert.SerializeObject(new { from = acc.Split('@')[1], OrderID = b.bbid, OrderRemark = b.msg });
                byte[] bytes = Encoding.UTF8.GetBytes(postdata);
                request.ContentLength = bytes.Length;
                Stream newStream = request.GetRequestStream();
                newStream.Write(bytes, 0, bytes.Length);
        ...
        public static bool changeStatus(betData b)
        {
            ...
            {
                ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;
                string url = urlbase + $"v2/api/merchant/merchantcenter/oc/manualchantOrder?tradeNo={b.bbid}&synStatus=6";
