...
namespace TimoControl
{
    public static class platGPK
    {
        ...
        public static bool submitToGPK(betData bb,string aname)
        {
            ...
            {
                //获取deposit_token
                string  ret_html = GetResponse<string>("Member/DepositToken", null, "POST", "MemberDeposit");
                if (ret_html==null)
                {
                    return false;
                }
                ...
                //提交数据
                string postUrl = "Member/DepositSubmit";
                string postData = "{\"AccountsString\":\"" + bb.username + "\",\"Type\":5,\"DepositToken\":\"" + ret_html + "\",\"AuditType\":\"None\",\"Amount\":" + bb.betMoney + ",\"IsReal\":false,\"PortalMemo\":\"" + aname + "-" + bb.gamename + "-" + bb.betno + "\",\"Memo\":\"" + aname + "-" + bb.gamename + "-" + bb.betno + "\",\"Password\":\"" + pwd + "\",\"TimeStamp\":" + (DateTime.Now.ToUniversalTime().Ticks - 621355968000000000) / 10000 + "}";
            ...
            {
                string msg = string.Format("GPK充值失败：用户 {0} 注单{1} {2} ", bb.username, bb.betno, ex.Message);
                appSittingSet.txtLog(msg);
                return false;
            }
